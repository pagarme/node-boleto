...
exports.barcodeData = (boleto) => {
  ...
  let nossoNumero = carteira + formatters.addTrailingZeros(boleto['nosso_numero'], 11);
  let barra = codigoBanco + numMoeda + fatorVencimento + valor + agencia + nossoNumero + codigoCedente + '0';
...
exports.parseEDIFile = (fileContent) => {
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      let line = lines[i];
      let registro = line.substring(0, 1);
      ...
      if (registro == '0') {
        ...
        let ocorrenciasStr = line.substring(318, 328);
        let motivosOcorrencia = new Array();
        ...
        for (let j = 0; j < ocorrenciasStr.length; j += 2) {
          let ocorrencia = ocorrenciasStr.substr(j, 2);
